<%--
addtophoto.jsp: redirect-get page adding photos for PhotoShare web app. 
	...
	<body>
		...
		<% 
			...
			try {
				...
				Picture picture = imageUploadBean.upload(request, user_id);
				String tag = ""; 
				PictureDao pictureDao = new PictureDao(); 
				if (picture != null) { 
					pictureDao.save(picture);
					
					tag  = picture.getTag(); 	//get tag
					if (tag != "") { 					//and add if there
						TagDao tagDao = new TagDao(); 
						tagDao.save(new Tag(pictureDao.loadLastSaved(), tag)); 
					}
					
				} else { 
					picture = new Picture(user_id, Integer.parseInt(request.getParameter("picture_id")), 
									request.getParameter("album_name"), request.getParameter("caption1"));
					
					tag = request.getParameter("tag"); 
					System.out.println("Here is the tag: " + tag); 
					if (tag != "") { 					//and add if there
						TagDao tagDao = new TagDao(); 
						tagDao.save(new Tag(picture.getId(), tag)); 
					}
			...
			if (err != null) {
				...
				out.println("<p>You've successfully updated your photos.</p>"); 
				out.println("</div>"); 
				out.println("</section>"); 
			}
